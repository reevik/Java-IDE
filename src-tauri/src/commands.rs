...
/// A task generated by the AI for the board.
#[derive(serde::Serialize)]
...
/// task list (title/description/column).
#[tauri::command]
pub async fn generate_tasks(
    ...
    // Extract the JSON array (the model may wrap it in prose/fences).
    let start = text.find('[').ok_or("The AI response didn't contain a task list.")?;
    let end = text.rfind(']').ok_or("The AI response didn't contain a task list.")?;
    if end <= start {
        return Err("Couldn't parse the AI task list.".into());
    }
    let raw: Vec<serde_json::Value> =
        serde_json::from_str(&text[start..=end]).map_err(|e| format!("Couldn't parse the AI task list: {e}"))?;

    let default_col = columns.first().cloned().unwrap_or_default();
    let tasks = raw
        .into_iter()
        .filter_map(|v| {
            ...
            let col = v.get("column").and_then(|s| s.as_str()).map(str::to_string).unwrap_or_default();
            // Snap to a known column (case-insensitive), else the first.
            let column = columns
                .iter()
