...
const TASKS_PROMPT: &str = "You break a feature request or description into a small set of actionable development tasks for a Trello-style board. Respond with ONLY a JSON array — no prose, no markdown, no code fences. Each element is an object: {\"title\": string (short, imperative, e.g. \"Add login endpoint\"), \"description\": string (one or two sentences of detail; may be empty), \"column\": string (exactly one of the provided column names), \"dependsOn\": string[] (titles of OTHER tasks in this same list that must be completed before this one can start; use the exact titles verbatim, [] if none)}. Produce between 3 and 8 tasks.\n\nDetermine the dependencies yourself from the natural order of the work: a task that needs another task's output must list it in \"dependsOn\" (e.g. \"Implement X\" depends on \"Locate where X lives\"; \"Add tests for X\" depends on \"Implement X\"). List the tasks in an order consistent with their dependencies (prerequisites first). Only reference titles that appear in this list, keep the dependency graph acyclic, and do not make a task depend on itself. Put tasks in the first column unless the description clearly implies another.";
